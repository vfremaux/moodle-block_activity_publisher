<?php //$Id: restore_check.php,v 1.1 2012-07-10 15:57:43 vf Exp $
...
//        DebugBreak();
    ...
    if (!empty($id)) {
        ...
        $restoreuserinfo = has_capability('moodle/restore:userinfo', context_course::instance($id));
    } else {
        if (!has_capability('moodle/site:restore', context_system::instance())) {
            error("You need to be an admin user to use this page.", "$CFG->wwwroot/login/index.php");
        }
        $restoreuserinfo = has_capability('moodle/restore:userinfo', context_system::instance());
    }
    ...
    if (empty($restore->course_id) && ($restore->restoreto == RESTORETO_EXISTING_DELETING || $restore->restoreto == RESTORETO_EXISTING_ADDING)) {
        ...
        if ($courses = $mycourses) {
            ...
            foreach ($courses as $course) {
                if (!has_capability('moodle/site:restore', context_course::instance($course->id))) {
                    continue;
                }
    ...
    //Checks everything and execute restore
        ...
        } else if (!empty($restore->course_id) and !has_capability('moodle/site:restore', context_course::instance($restore->course_id))) {
        ...
        if ($restore->course_startdateoffset != 0) {
            ...
            if (!empty($restore->course_id)) {
                $canrolldates = has_capability('moodle/restore:rolldates', context_course::instance($restore->course_id));
            // if we know the target category, check capability on it
            } else if (!empty($restore->restore_restorecatto)) {
                $canrolldates = has_capability('moodle/restore:rolldates', context_coursecat::instance($restore->restore_restorecatto));
            }
       ...
       if ($restore->users == 2) {
        ...
        if ($restore->users == 0 and !has_capability('moodle/site:restore', context_system::instance())) {
        ...
        if (!empty($restore->course_id)) {
            $contexttosearch = context_course::instance($restore->course_id);
        // if we know the target category, check capability on it
        } else if (!empty($restore->restore_restorecatto)) {
            $contexttosearch = context_coursecat::instance($restore->restore_restorecatto);
        } else {
            $contexttosearch = context_system::instance();
        }
        $cancreateroles = has_capability('moodle/role:manage', context_system::instance());
    ...
    /// Warning and button
        ...
        /// Restore button
            print_single_button("restore.php", $hidden, get_string("restoreactivitynow","block_activity_publisher"),"post");
        }
        echo "</div>";
