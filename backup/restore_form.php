<?php //$Id: restore_form.php,v 1.1 2012-07-10 15:57:44 vf Exp $
    ...
    if ($allmods = get_records("modules") ) {
       ...
       /*
        ...
        if (($info->backup_users == "all" or $info->backup_users == "course") and $restoreuserinfo) {
            ...
            if ($info->backup_users == "all" and has_capability('moodle/site:restore', context_system::instance())) {
...
if ($restoreuserinfo) {
    ...
    $siterolesarray = get_assignable_roles (context_course::instance($course->id), "shortname", ROLENAME_ORIGINAL);
    $siterolesnamearray = get_assignable_roles (context_course::instance($course->id), "name", ROLENAME_ORIGINAL);
    ...
    if ($info->backup_moodle_version < 2006092801) {
        ...
        $roles = get_roles_with_capability('moodle/legacy:editingteacher', CAP_ALLOW, context_system::instance());
        ...
        // get the first teacheredit legacy
        $roles = get_roles_with_capability('moodle/legacy:teacher', CAP_ALLOW, context_system::instance());
        $teacher = reset($roles);
        ...
        // get the first teacheredit legacy
        $roles = get_roles_with_capability('moodle/legacy:student', CAP_ALLOW, context_system::instance());
        ...
        if (!empty($roles->roles)) { // possible to have course with no roles
            ...
            foreach ($roles->roles as $roleid=>$role) {
                ...
                if ($samerole = restore_samerole($roleid, $role)) {
                    ...
                    if ($matchrole == 0 && !has_capability('moodle/role:manage', context_system::instance())) {
