<?php // $Id: backuplib.php,v 1.4 2012-07-22 20:28:51 vf Exp $
/**
...
//This is the "graphical" structure of the question database:
    ...
    function ap_backup_question_categories($bf, $preferences, $question_cats_ids_arr, $questions_ids_arr) {
        ...
        /*
        echo "table registered ";
        print_object($categories);
        
        echo "expected ";
        print_object($question_cats_ids_arr);

        echo "up to store ap_backup_question_categories() ";
        die;
        */
...
//                DebugBreak(); 
	...
	/**
	* could be used to store files related to questions
	*
	*/
       ...
       if( !empty($qt_filemap[$table_name]) && count($qt_filemap[$table_name]) > 0 ){
         	foreach($qt_filemap[$table_name] as $field => $value){
            	...
            	if ($qt_filemap[$table_name][$field] == 'embedded'){
               		ap_backup_copy_question_files_from_content($fvalue, $preferences);// scan content for any files  
            	} else {
	             	...
	             	if(file_exists($rootdir.'/'.$fvalue) && $fvalue != '' && $fvalue != null){
	                    ...
	                    $status = backup_copy_file($rootdir.'/'.$fvalue, $CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/questions_files/'.$filename);
	             	}	elseif(file_exists($CFG->dataroot.'/'.SITEID.'/'.$fvalue) && $fvalue!= '' && $fvalue != null) {
	                  	//looks like its not in the course , we try in the site files.
	                  	$status = backup_copy_file($CFG->dataroot.'/'.SITEID.'/'.$fvalue, $CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/questions_files/'.$fvalue);
	             	}
	            }            
         	}  
       	}
    ...
    function ap_backup_copy_question_files_from_content($content, $preferences) {
        global $CFG;

        $status = true;
 
        $status = ap_check_and_create_questions_files_dir($preferences->backup_unique_code);
      
        //copied to backup
        $files_to_export = array();
        $rootdir = $CFG->dataroot;
                        
        $files_to_export = ap_list_question_files($content, $preferences);    
                ...
                foreach ($files_to_export as $dir) {
                    ...
                    if ( $dir !== $name_backupdata && $dir !== $name_moddata){             
                        if (file_exists($rootdir.'/'.$dir)) {
                            ...
                            $status = backup_copy_file($rootdir.'/'.$dir, $CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/questions_files/'.$filename);
                        }                            
                     }
                }
            }
        ...
        if ($dirs) {
            ...
            foreach ($dirs as $dir) {
	            ...
	            if ( $dir !== $name_backupdata && $dir !== $name_moddata){
	                ...
	                $status = $status && backup_copy_file($rootdir.'/'.$dir, $CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/questions_files/'.$dir);
	                }
	            }
	        }
	    }       
    ...
    */
    function ap_list_question_files ($content , $preferences){ 
    	global $CFG;
    ...
    function ap_check_and_create_questions_files_dir($backup_unique_code) {
        global $CFG;
    ...
    function ap_question_category_ids_by_backup ($backup_unique_code) {
        ...
        return get_records_sql ("
        	SELECT 
        		...
        		a.backup_code
			FROM 
				{$CFG->prefix}backup_ids a
			WHERE 
				a.backup_code = '$backup_unique_code' AND
    ...
    function ap_question_ids_by_backup ($backup_unique_code) {
        ...
        return get_records_select ('backup_ids', " backup_code = '{$backup_unique_code}' AND table_name = 'question' ", 'old_id,backup_code');
    }
    ...
    function ap_question_insert_c_and_q_ids_for_course($coursecontext, $backup_unique_code){
        ...
        $status = execute_sql("
			...
			WHERE 
				qc.contextid = {$coursecontext->id}", false);
    ...
    function ap_question_insert_q_ids($backup_unique_code, $info){
        ...
        $status = execute_sql("
			...
			FROM 
				...
				{$CFG->prefix}backup_ids bk
			WHERE 
				q.category = bk.old_id AND 
				bk.table_name = 'question_categories' AND 
				" . sql_compare_text('bk.info') . " = '$info' AND 
    ...
    function ap_question_insert_c_and_q_ids_for_module($backup_unique_code, $course, $modulename, $instances){
        ...
        $cmcontexts = array();
        if(!empty($instances)) {
            $cmcontexts = get_records_sql_menu("
            	SELECT 
            		c.id, 
            		c.id AS dummykeyname 
            	FROM 
				...
				WHERE 
					...
					m.id = cm.module AND 
					cm.id = c.instanceid AND 
					c.contextlevel = ".CONTEXT_MODULE." AND 
					cm.course = $course AND 
					cm.instance IN (".implode(',',array_keys($instances)).")");
        }
                                    
        if ($cmcontexts){
            $status = $status && execute_sql("
            	INSERT INTO 
            		{$CFG->prefix}backup_ids(
            			backup_code, 
            			table_name, 
				...
				FROM 
					{$CFG->prefix}question_categories qc
				WHERE 
					qc.contextid IN (".join(array_keys($cmcontexts), ', ').")", false);
        }
        $status = $status && ap_question_insert_q_ids($backup_unique_code, $modulename);
        return $status;
    }

    function ap_question_insert_course_file_names($courseid, $backup_unique_code){
        global $QTYPES, $CFG;
        $status = true;
        ...
        if ($questionids){
            foreach ($questionids as $question_bk){
                ...
                $qurls = $QTYPES[$question->qtype]->find_file_links($question, $courseid);
                $urls = array_merge_recursive($urls, $qurls);
            }
        }
        ksort($urls);
        foreach (array_keys($urls) as $url){
            if (file_exists($CFG->dataroot.'/'.$courseid.'/'.$url)){
    ...
    function ap_question_insert_site_file_names($backup_unique_code){
        global $QTYPES, $CFG;
        $status = true;
        $questionids = ap_question_ids_by_backup($backup_unique_code);
        $urls = array();
        if ($questionids){
            foreach ($questionids as $question_bk){
                $question = get_record('question', 'id', $question_bk->old_id);
                $QTYPES[$question->qtype]->get_question_options($question);
                $qurls = $QTYPES[$question->qtype]->find_file_links($question, SITEID);
                $urls = array_merge_recursive($urls, $qurls);
            }
        }
        ksort($urls);
