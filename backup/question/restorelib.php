<?php // $Id: restorelib.php,v 1.3 2012-07-17 20:35:30 vf Exp $
/**
...
//This is the "graphical" structure of the question database:
    ...
    */
    function ap_restore_question_get_best_category_context($restore, $contextinfo) {
        ...
        switch ($contextinfo['LEVEL'][0]['#']) {
            case 'module':
                ...
                $tocontext = context_module::instance($instanceinfo->new_id);
                break;
            case 'course':

                $tocontext = context_course::instance($restore->course_id);
                break;
            case 'coursecategory':
                ...
                $tocontext = context_coursecat::instance($catid);
                */
                //SOLUTION 1 : just move the questions from the course category level to the 
                //             destination course level.
                $tocontext = context_course::instance($restore->course_id);

                break;
            case 'system':
                $tocontext = context_system::instance();
                break;
        }
        return $tocontext;
    }

    function ap_restore_question_categories($info, $restore) {
        $status = true;
        //Iterate over each category
        foreach ($info as $category) {
            $status = $status && ap_restore_question_category($category, $restore);
        }
        $status = $status && ap_restore_recode_category_parents($restore);
        return $status;
    }

    function ap_restore_question_category($category, $restore) {
        $status = true;
            ...
            if ($data) {
                ...
                if (isset($info['QUESTION_CATEGORY']['#']['PUBLISH'])) {
                    ...
                    if ($publish){
                        ...
                        $tocontext = context_course::instance($course);
                    }
                } else {
                    if (!$tocontext = ap_restore_question_get_best_category_context($restore, $info['QUESTION_CATEGORY']['#']['CONTEXT']['0']['#'])){
                ...
                if ($question_cat->id) {
                    ...
                    $status = ap_restore_questions($category->id, $question_cat, $info, $restore);
                } else {
                    $status = false;
                }
                if (!defined('RESTORE_SILENTLY')) {
    ...
    function ap_restore_recode_category_parents($restore){
        global $CFG;
        $status = true;
    ...
    function ap_restore_questions ($old_category_id, $best_question_cat, $info, $restore) {
        ...
        for($i = 0; $i < sizeof($questions); $i++) {
            ...
            $question->questiontext = ap_backup_html_todb($que_info['#']['QUESTIONTEXT']['0']['#']);
            $question->questiontextformat = backup_todb($que_info['#']['QUESTIONTEXTFORMAT']['0']['#']);
            $question->image = backup_todb($que_info['#']['IMAGE']['0']['#']);
            $question->generalfeedback = html_entity_decode(ap_backup_todb_optional_field($que_info, 'GENERALFEEDBACK', ''), ENT_QUOTES, 'UTF-8' );
            ...
            $question->timecreated = ap_backup_todb_optional_field($que_info, 'TIMECREATED', 0);
            $question->timemodified = ap_backup_todb_optional_field($que_info, 'TIMEMODIFIED', 0);

            // Set the createdby field, if the user was in the backup, or if we are on the same site.
            $createdby = ap_backup_todb_optional_field($que_info, 'CREATEDBY', null);
            if (!empty($createdby)) {
                ...
                if ($user) {
                    ...
                    $question->createdby = $USER->id; // Change from standard : reappopriate question to current course owner
                }
            }

            // Set the modifiedby field, if the user was in the backup, or if we are on the same site.
            $modifiedby = ap_backup_todb_optional_field($que_info, 'MODIFIEDBY', null);
            if (!empty($createdby)) {
                $user = backup_getid($restore->backup_unique_code, 'user', $modifiedby);
                if ($user) {
                    $question->modifiedby = $user->new_id;
                } else if (backup_is_same_site($restore)) {
                    $question->modifiedby = $USER->id; // Change from standard : reappopriate question to current course owner 
                }
            }
            ...
            if ($existingquestion) {
                ...
                $question->questiontext = $question_copy->questiontext;
                if (empty($question->createdby)) $question->createdby = $USER->id;
                if (empty($question->modifiedby)) $question->modifiedby = $USER->id;
                
                update_record('question', addslashes_object($question));
                ...
                $bestcontext = context::instance_by_id($best_question_cat->contextid);
                if (($bestcontext->contextlevel == CONTEXT_SYSTEM ||  $bestcontext->contextlevel == CONTEXT_COURSECAT)
                    ...
                    if (!isset($course_question_cat)) {
                        $coursecontext = context_course::instance($restore->course_id);
        ...
        /// If it's a new question in the DB, restore it
            ...
            /// We have to recode the parent field
                ...
                $status = ap_question_restore_answers($oldid,$newid,$que_info,$restore);
                ...
                //in each attempt.
                $status = ap_question_restore_map_answers($oldid,$newid,$que_info,$restore);
    ...
    function ap_backup_todb_optional_field($data, $field, $default) {
	...
	function ap_backup_html_todb($data){
		return str_replace('\"', '"', html_entity_decode(backup_todb($data), ENT_QUOTES, 'UTF-8' ));
	}

    function ap_question_restore_answers ($old_question_id,$new_question_id,$info,$restore) {
        ...
        if (isset($info['#']['ANSWERS']['0']['#']['ANSWER'])) {
            ...
            for($i = 0; $i < sizeof($answers); $i++) {
                ...
                $answer->answer = ap_backup_html_todb($ans_info['#']['ANSWER_TEXT']['0']['#']);
                $answer->fraction = backup_todb($ans_info['#']['FRACTION']['0']['#']);
                $answer->feedback = ap_backup_html_todb($ans_info['#']['FEEDBACK']['0']['#']);
    ...
    function ap_question_restore_map_answers ($old_question_id,$new_question_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($answers); $i++) {
            ...
            $db_answer = get_record("question_answers","question",$new_question_id, "answer", $answer->answer);
    ...
    function ap_question_restore_numerical_units($old_question_id,$new_question_id,$info,$restore) {
    ...
    function ap_question_restore_dataset_definitions ($old_question_id,$new_question_id,$info,$restore) {
    ...
    function ap_question_restore_dataset_items ($definitionid,$info,$restore) {
    ...
    function ap_question_states_restore_mods($attempt_id,$info,$restore) {
     ...
     */
    function ap_question_decode_content_links_caller($restore) {
        global $CFG, $QTYPES;
        $status = true;
        ...
        $context = context_course::instance($restore->course_id);
