...
define('RESTORE_GROUPS_GROUPINGS', 3);
    ...
    */
    ...
    //    - $@FILEPHP@$ ---|------------> $CFG->wwwroot/file.php/courseid/moddata/modtype/modinstance (slasharguments on)
    ...
    /// heavy interlinking. Just a few cpu cycles saved.
        ...
        $search = ('$@FILEPHP\@$');                 
        $filepath_base = array(get_file_url($restore->course_id));
 //       $result = str_replace($search,$replace,$content);

		$result = $content;

        if(@$restore->mods['quiz']->instances > 0){           
             foreach($restore->mods['quiz']->instances as $mod){
             	$quiz_instance_id =  $mod->restored_as_course_module;             	
             	$path_to_moddata = "/".$CFG->moddata."/quiz/".$quiz_instance_id;    
		        $filepath = $filepath_base[0].$path_to_moddata;		        
		        $result = preg_replace($search, $filepath, $result);  
             }              
        }
   ...
   //restore the activity questions files . 
     ...
     */
    function restore_precheck($id,$file,&$errorstr,$noredirect=false,$backup_unique_code) {
...
*/
        ...
        */
        ...
        /* ($status and ($info->backup_logs == 'true' && $restore->logs)) {
            if (!defined('RESTORE_SILENTLY')) {
