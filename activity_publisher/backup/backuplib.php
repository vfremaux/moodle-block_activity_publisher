<?php //$Id: backuplib.php,v 1.3 2012-07-17 20:35:29 vf Exp $
...
//          DebugBreak();
    ...
    // In this way we enable for max integration. 
    ...
    // - Legacy Entry Point  : look for $module_backup_one_mod in the mod backup file [modname/backuplib.php]
        ...
        else if(file_exists($CFG->dirroot.'/block/activity_publisher/backup/'.$module.'/backuplib.php'))
        {
            require_once($CFG->dirroot.'/block/activity_publisher/backup/'.$module.'/backuplib.php');
        }
    ...
    function backup_process_filephp_uses($matches) {
        ...
        $m2_arr = split("/",$matches[2]);
        $matches[2] = $m2_arr[count($m2_arr)-1];
       
        if(isset($matches[2])) 
        { 
            $content1 = str_replace($search, $replace, $matches[2]) ;
        }
        else{
            $content1 = '';
        }
        ...
        if(isset($matches[3]))
        {
          $content2 = str_replace($search, $replace, $matches[3])  ;
        }
        else{
          $content2 = '';  
        }
        
        $result = $matches[1] . $content1 . $content2 ;
    ...
    function backup_copy_course_files ($preferences) {
        ...
        if (is_dir($rootdir)) {
            ...
            $files_to_export = list_questions_files($preferences);
     ...
     */
    function backup_fetch_prefs_from_request(&$preferences,&$count,$course,$mod_id) {
        global $CFG, $SESSION;
    ...
    function backup_execute(&$preferences, &$errorstr) {
       ...
       /* if (!defined('BACKUP_SILENTLY')) {
        ...
        if ($status) {
         ...
         //       DebugBreak();
                ...
                if(!empty($preferences->quiz_instances) ){
                    
                    foreach($preferences->quiz_instances as $instance){
                       	$quiz_question_instances = get_records('quiz_question_instances', 'quiz', $instance->id, 'id'); 
                       	if(!empty($quiz_question_instances)){
                           	foreach($quiz_question_instances as $q_ins){
                              	$category = get_field('question', 'category', 'id', $q_ins->question) ;
                              	$quiz_questions_ids[] = $q_ins->question;
                              	$quiz_question_categories_id[] = $category; 
                              	...
                              	if ($children = get_records('question', 'parent', $q_ins->question, 'id', 'id,category')){
	                           		foreach($children as $q_ins){
		                              	$quiz_questions_ids[] = $q_ins->id;
		                              	$quiz_question_categories_id[] = $q_ins->category; 
	                           		}
	                           	}
                           	}
						}                       
                ...
                if (!$status = ap_backup_question_categories($backup_file, $preferences,$quiz_question_categories_id,$quiz_questions_ids)) {
                    if (!defined('BACKUP_SILENTLY')) {
     ...
     /*    if ($status) {
         //   if ($preferences->backup_site_files) {
                if (!defined('BACKUP_SILENTLY')) {
                    echo "<li>".get_string('copyingquestionsfiles', 'block_activity_publisher').'</li>';
                }
                if (!$status = backup_copy_questions_files ($preferences)) {
                    if (!defined('BACKUP_SILENTLY')) {
    ...
    */
    function backup_get_zipfile_name($module, $course_mod) {
